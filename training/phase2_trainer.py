...
def _train_epoch(model: CommitRankingModule, loader: DataLoader,
    ...
    for b_idx, batch in enumerate(loader):
        if not batch:
            continue
        b_loss, b_count = 0.0, 0

        for sample in batch:
            try:
                scores, _ = model(
                    sample["node_embeddings"].to(device),
                    sample["commit_indices"].to(device),
                    sample["num_commits"],
                )
                loss = loss_fn(scores, sample["ground_truth_positions"]) / grad_accum
                if loss.requires_grad and not torch.isnan(loss):
                    loss.backward()
                    b_loss += loss.item() * grad_accum
                    b_count += 1
                with torch.no_grad():
                    for k, v in compute_metrics(scores, sample["ground_truth_positions"]).items():
                        all_m[k].append(v)
    ...
    if total_samples and b_idx >= 0 and (b_idx + 1) % grad_accum != 0:
        clip_and_step(model, optimizer)
        ...
        for b_idx, batch in enumerate(loader):
            ...
            for sample in batch:
                try:
                    ...
                    loss = loss_fn(scores, sample["ground_truth_positions"])
...
def train_phase2_fold(fold_idx: int, train_indices: List[int],
    ...
    nw = max(config.get("num_workers", 0), 2)
    loader_kw = dict(collate_fn=collate_phase2, num_workers=nw,
                     pin_memory=(primary.type == "cuda"),
                     persistent_workers=(nw > 0))
    print(f"  DataLoader: num_workers={nw} (prefetch batches in parallel)")
